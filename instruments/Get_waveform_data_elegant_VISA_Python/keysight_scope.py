...
class Oscilloscope():
    ...
    # private methods to use Oscilloscope within context manager
    def __enter__(self):
    ...
    def acquire_traces(self):
        ...
        self.scope_obj.write(':STOP')
        # Get Number of analog channels on scope
        IDN = str(self.scope_obj.query("*IDN?"))
        # Parse IDN
        ...
        self.scope_obj.write(":WAVeform:POINts:MODE MAX") # MAX mode works for all acquisition types, so this is done here to avoid Acq. Type vs points mode problems. Adjusted later for specific acquisition types.

        ch = 1 # Channel number
        for each_value in CHS_LIST:
            On_Off = int(self.scope_obj.query(":CHANnel" + str(ch) + ":DISPlay?")) # Is the channel displayed? If not, don't pull.
            if On_Off == 1: # Only ask if needed... but... the scope can acquire waveform data even if the channel is off (in some cases) - so modify as needed
                Channel_Acquired = int(self.scope_obj.query(":WAVeform:SOURce CHANnel" + str(ch) + ";POINts?")) # If this returns a zero, then this channel did not capture data and thus there are no points
            ...
            if Channel_Acquired == 0 or On_Off == 0: # Channel is off or no data acquired
                self.scope_obj.write(":CHANnel" + str(ch) + ":DISPlay OFF") # Setting a channel to be a waveform source turns it on... so if here, turn it off.
                ...
                Pre = self.scope_obj.query(":WAVeform:PREamble?").split(',') # ## The programmer's guide has a very good description of this, under the info on :WAVeform:PREamble.
                ...
                CH_UNITS[ch-1] = str(self.scope_obj.query(":CHANnel" + str(ch) + ":UNITs?").strip('\n')) # This isn't really needed but is included for completeness
            ch += 1
        ...
        self.scope_obj.write(":WAVeform:FORMat WORD") # 16 bit word format... or BYTE for 8 bit format - WORD recommended, see more comments below when the data is actually retrieved
            # WORD format especially  recommended  for Average and High Res. Acq. Types, which can produce more than 8 bits of resolution.
        self.scope_obj.write(":WAVeform:BYTeorder LSBFirst") # Explicitly set this to avoid confusion - only applies to WORD FORMat
        self.scope_obj.write(":WAVeform:UNSigned 0") # Explicitly set this to avoid confusion
        ...
        ACQ_TYPE = str(self.scope_obj.query(":ACQuire:TYPE?")).strip("\n")
        ...
        self.scope_obj.write(":WAVeform:SOURce CHANnel" + str(FIRST_CHANNEL_ON))
        ...
        self.scope_obj.write(":WAVeform:POINts MAX") # This command sets the points mode to MAX AND ensures that the maximum # of points to be transferred is set, though they must still be on screen
        ...
        self.scope_obj.write(":WAVeform:POINts:MODE " + str(POINTS_MODE))
        ...
        MAX_CURRENTLY_AVAILABLE_POINTS = int(self.scope_obj.query(":WAVeform:POINts?")) # This is the max number of points currently available - this is for on screen data only - Will not change channel to channel.
        # NOTES:
        ...
        self.scope_obj.write(":WAVeform:POINts " + str(USER_REQUESTED_POINTS))

        # Then ask how many points it will actually give you, as it may not give you exactly what you want.
        NUMBER_OF_POINTS_TO_ACTUALLY_RETRIEVE = int(self.scope_obj.query(":WAVeform:POINts?"))
        ...
        Pre = self.scope_obj.query(":WAVeform:PREamble?").split(',') # This does need to be set to a channel that is on, but that is already done... e.g. Pre = self.scope_obj.query(":WAVeform:SOURce CHANnel" + str(FIRST_CHANNEL_ON) + ";PREamble?").split(',')
        ...
        WFORM = str(self.scope_obj.query(":WAVeform:FORMat?"))
        if WFORM == "BYTE":
            FORMAT_MULTIPLIER = 1
        else: #WFORM == "WORD"
            FORMAT_MULTIPLIER = 2
        ...
        if TOTAL_BYTES_TO_XFER >= 400000:
            self.scope_obj.chunk_size = TOTAL_BYTES_TO_XFER
        # else:
        ...
        for channel_number in CHS_ON:
            ...
            # The below method uses an IEEE488.2 compliant definite length binary block transfer invoked by :WAVeform:DATA?.
                ...
                Wav_Data[:,i] = np.array(self.scope_obj.query_binary_values(':WAVeform:SOURce CHANnel' + str(channel_number) + ';DATA?', "h", False)) # See also: https://PyVisa.readthedocs.io/en/stable/rvalues.html#reading-binary-values
                ...
                # The :WAVeform:DATA? query can be interrupted without an error by doing a device clear: self.scope_obj.clear()
        ...
        if TOTAL_BYTES_TO_XFER >= 400000:
            self.scope_obj.chunk_size = 20480
